...
import java.io.File;
/*
Repita a questão de Ordenação por Seleção, contudo,
usando o algoritmo de Inserção, fazendo com que a chave de pesquisa seja o atributo fundaca-
oAno. O nome do arquivo de log será matrı́cula insercao.txt.
*/
...
class TimeInsercao {
    ...
    public TimeInsercao(String nomeArq) throws Exception{
        ler(nomeArq);
    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        TimeInsercao[] conjuntoTimes = new TimeInsercao[quantidadeDeFrases];
        
        //criar os objetos de acordo.
        for (int i = 0; i < quantidadeDeFrases; i++) {
            //criar o objeto e chamar o construtor
            conjuntoTimes[i] = new TimeInsercao(entradas[i]);
        }
        
        int totalComparacoes = ordenarInsercao(conjuntoTimes);
        ...
        Arq.openWrite("649651_insercao.txt");
    ...
    public static int ordenarInsercao(TimeInsercao array[]){
        int comparacoes = 0;

        int n = array.length;

        for (int i = 1; i < n; i++) {
            TimeInsercao tmp = array[i];
            int j = i - 1;
            ...
            while ( (j >= 0) && (array[j].getFundacaoAno() > tmp.getFundacaoAno())){
                ...
                if(array[j].getFundacaoAno() == tmp.getFundacaoAno()){
                    if(array[j].getFundacaoMes() > tmp.getFundacaoMes()){
                        array[j + 1] = array[j];
                    }                    
                }
                else{
